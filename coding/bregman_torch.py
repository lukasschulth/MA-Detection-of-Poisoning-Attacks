...
def sinkhorn_torch(a, b, M, reg, method='sinkhorn_stabilized', numItermax=1000,
    ...
    if method.lower() == 'sinkhorn':
        return sinkhorn_knopp_torch(a, b, M, reg, numItermax=numItermax,
...
def sinkhorn_knopp_torch(a, b, M, reg, numItermax=1000,
                   stopThr=1e-9, verbose=False, log=False, **kwargs):
    r"""
    Solve the entropic regularization optimal transport problem and return the OT matrix
    ...
    if len(a) == 0:
        a = torch.ones(len(M.shape[0]), dtype=torch.float64) / M.shape[0]
    if len(b) == 0:
        b = torch.ones(len(M.shape[1]), dtype=torch.float64) / M.shape[1]
    ...
    # distances
    if n_hists:
        u = torch.ones((dim_a, n_hists), dtype=torch.float64) / dim_a
        v = torch.ones((dim_b, n_hists), dtype=torch.float64) / dim_b
    else:
        u = torch.ones(dim_a, dtype=torch.float64) / dim_a
        v = torch.ones(dim_b, dtype=torch.float64) / dim_b
    ...
    while (err > stopThr and cpt < numItermax):
        uprev = u
        vprev = v
        #print((np.dot(K.T.cpu().numpy(), u.cpu().numpy())).shape)
        #print(b.shape)
        KtransposeU = torch.matmul(K.T, u)
        #print('KTUSHAPE; ', KtransposeU.shape)
        #print(b.shape)
        ...
        #TODO: wegen torch einsum habe ich das Stoppkriterium einfach übersprungen, das sollrte ich noch fixen
        if cpt % 100000 == 0:
